...
export default function AllPostsSection({ selectedDate, searchQuery }: AllPostsSectionProps) {
    ...
    return (
      <div className="backdrop-blur-md bg-white/10 dark:bg-white/5 border border-white/20 dark:border-white/10 rounded-2xl p-4 sm:p-6 shadow-xl text-center">
        ...
        <p className="text-xs sm:text-sm text-muted-foreground">
          {searchQuery ? "No posts match your search" : selectedDate === "all" ? "No posts found" : "No posts found on this date"}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="grid gap-3 sm:gap-4 md:gap-6">
      <div className="mb-2 sm:mb-4">
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-accent drop-shadow-lg">All Posts</h2>
      ...
      {filteredPosts.map((post) => {
        ...
        const colors = platformColors[post.platform] || platformColors['other']
